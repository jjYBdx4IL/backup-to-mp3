...
* everything else OFF
* enable header mode: OFF - we repeat the transmission multiple times instead - a slower header mode could theoretically provide some stability, but it might also be the cause for fldigi's direct audio capture to stop.
...
* Nothing special. Again, don't use input and output of the virtual cable at the same time.
...
* attic/ folder contains my previous evaluation attempts before I ended up using the final solution.
